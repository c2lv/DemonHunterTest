...
public static class StringExtensions
{
    ...
    {
        path = Path.Join(MainFile.ResPath, "images", path);
        if (!ResourceLoader.Exists(path)) MainFile.Logger.Info("Could not find image path: " + path);
        return path;
    }
    ...
    public static string CharacterUiPath(this string path)
    {
        ...
        if (!ResourceLoader.Exists(path)) MainFile.Logger.Info("Could not find character UI path: " + path);
        return path;
    }
    ...
    public static string PotionImagePath(this string path)
    {
        ...
        if (!ResourceLoader.Exists(path)) MainFile.Logger.Info("Could not find potion image path: " + path);
        return path;
    }
    public static string AnimationPath(this string path)
    {
        path = Path.Join(MainFile.ResPath, "animations", path);
        if (!ResourceLoader.Exists(path)) MainFile.Logger.Info("Could not find animation path: " + path);
        return path;
    }
    public static string ScenesPath(this string path)
    {
        path = Path.Join(MainFile.ResPath, "scenes", path);
        if (!ResourceLoader.Exists(path)) MainFile.Logger.Info("Could not find scene path: " + path);
        return path;
    }
}